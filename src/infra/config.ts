 ...
 *
 * Reads `pidev.json` from two places and merges them. Project values normally
 * override global values; permissions compose restrictively:
 *
 ...
 * typo never takes the harness down. Most fields use the generic merge below;
 * security-sensitive permissions receive restrictive post-processing.
 */
...
export interface RoutingModelConfig {
    ...
    /** Router stickiness for this target in minutes. Does not guarantee an upstream cache. */
...
export interface RoutingPreset {
    ...
    /** Intermediate recovery model after base struggles. */
    escalated?: RoutingModelSetting;
    /** Final recovery model. */
    deep?: RoutingModelSetting;
    /** Router stickiness for every target in this preset, unless a target overrides it. */
    cacheTtlMinutes?: number;
}
...
export interface RoutingConfig {
    ...
    fast?: RoutingModelSetting;
    /** Legacy intermediate recovery model after base struggles. */
    escalated?: RoutingModelSetting;
    /** Legacy final recovery model. */
    deep?: RoutingModelSetting;
    /** Consecutive failed recovery-rung turns before advancing. Default 2. */
    failureThreshold?: number;
    /** User correction requests on one recovery rung before advancing. Default 2. */
    correctionThreshold?: number;
    /** Repeated remediated development failures before they become stagnation. Default 2. */
...
export interface PiDevConfig {
    ...
    timer?: {
        enabled?: boolean;
        /**
         * Rotating verbs for the Working label, picked fresh each agent run.
         * Defaults to the built-in 200-verb list (src/domain/working-verbs.ts);
         * a non-empty array replaces it; false keeps the plain Working text.
         */
        workingVerbs?: string[] | false;
    };
    ...
    /** Permission mode and per-effect decisions. Project values may only tighten global policy. */
    ...
    /** Automatic fast/base/escalated/deep routing. */
    routing?: RoutingConfig;
    /**
     * Proactive compaction at a settled task boundary, ahead of pi's own
     * overflow-time trigger. On by default; see domain/auto-compact.ts for the
     * bands. Tokens are absolute counts, percents are fractions of the window.
     */
    autoCompact?: {
        enabled?: boolean;
        /** Never compact below this many context tokens. Default 40000. */
        minTokens?: number;
        /** Task-boundary band. Defaults 0.7 of the window, capped at 180000 tokens. */
        softPercent?: number;
        softCeilingTokens?: number;
        /** Mid-task band. Defaults 0.85 of the window, capped at 220000 tokens. */
        hardPercent?: number;
        hardCeilingTokens?: number;
        /** Assistant turns to let pass after a compaction. Default 2. */
     ...
     */
    secretGuard?: {
        /** Supplemental guard mode: "block" (default), "warn", or "off". */
        mode?: "off" | "warn" | "block";
...
export function loadConfig(cwd: string): PiDevConfig {
    ...
    const permissions = mergePermissionConfigs(
        normalizePermissionConfig(global.permissions),
        normalizePermissionConfig(project.permissions),
    );
    const merged = mergeObjects(global, project);
    const { permissions: _untrustedPermissions, ...rest } = merged;

    // Trust boundary: the merged tree is JSON in the pidev.json shape (all fields optional).
    return (permissions === undefined ? rest : { ...rest, permissions }) as PiDevConfig;
}
