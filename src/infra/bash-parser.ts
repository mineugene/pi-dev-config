...
/**
 * Character devices and fd aliases that hold no file content: exempt from
 * path candidates. Kernel pseudo-filesystems (/proc, /sys) and block devices
 * stay gated because they can expose secrets or raw storage.
 */
...
function classifyTokenAsPathCandidate(token: string): string | null {
    ...
    if (PSEUDO_DEVICES.has(token)) return null;
