...
import { directoryStatus, resolveSearchScope } from "./search-scope.ts";
...
describe("directoryStatus", () => {
    it("distinguishes directories, other entries, and unavailable paths", () => {
        expect(directoryStatus(external)).toBe(true);
        expect(directoryStatus(path.join(external, "flake.nix"))).toBe(false);
        expect(directoryStatus(path.join(external, "missing"))).toBeUndefined();
    });
});
