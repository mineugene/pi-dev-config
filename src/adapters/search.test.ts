...
describe("search token bounds", () => {
    ...
    it("coaches bash extraction toward rg over sed", () => {
        const tools: SearchTool[] = [];
        registerSearch({
            registerTool: (tool: SearchTool) => tools.push(tool),
        } as unknown as ExtensionAPI);
        const guidelines = findTool(tools, "grep").promptGuidelines ?? [];
        const guideline = guidelines.find((line) => line.includes("rg -o"));
        expect(guideline).toBeDefined();
        expect(guideline).toContain("sed");
        expect(guideline).toContain("prompts for approval");
    });
