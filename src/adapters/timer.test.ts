...
describe("elapsed timer", () => {
    ...
    it("shows and finalises the working timer with a rotating verb", async () => {
        const timer = setup({ timer: { workingVerbs: ["Brewing"] } });

        await timer.emit("session_start");
        await timer.emit("agent_start");
        expect(timer.setWorkingMessage).toHaveBeenLastCalledWith("Brewing... 0ms");

        await vi.advanceTimersByTimeAsync(862);
        await timer.emit("agent_end");
        expect(timer.setWorkingMessage).toHaveBeenLastCalledWith("Brewing... 862ms");

        await timer.emit("agent_settled");
        expect(vi.getTimerCount()).toBe(0);
    });

    it("falls back to the plain Working text when verbs are disabled", async () => {
        const timer = setup({ timer: { workingVerbs: false } });
    ...
    it("refreshes the working label at most once every 200 milliseconds", async () => {
        const timer = setup();

        await timer.emit("session_start");
        await timer.emit("agent_start");
        expect(timer.setWorkingMessage).toHaveBeenCalledTimes(1);

        await vi.advanceTimersByTimeAsync(199);
        expect(timer.setWorkingMessage).toHaveBeenCalledTimes(1);

        await vi.advanceTimersByTimeAsync(1);
        expect(timer.setWorkingMessage).toHaveBeenCalledTimes(2);
    });
    ...
    it("renders only the timer line even when thinking contains heading-like text", async () => {
        const timer = setup();
        await startFirstTurn(timer);
    ...
    it("keeps the verb when the timer is disabled", async () => {
        const timer = setup({ timer: { enabled: false, workingVerbs: ["Brewing"] } });
        ...
        expect(timer.setWorkingMessage).toHaveBeenCalledTimes(1);
        expect(timer.setWorkingMessage).toHaveBeenLastCalledWith("Brewing...");
        expect(timer.setHiddenThinkingLabel).not.toHaveBeenCalled();
        expect(timer.entries).toEqual([]);
        expect(vi.getTimerCount()).toBe(0);
    });

    it("is fully off when both the timer and the verbs are disabled", async () => {
        const timer = setup({ timer: { enabled: false, workingVerbs: false } });
