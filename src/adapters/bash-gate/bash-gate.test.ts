...
describe("extractBashFacts", () => {
    ...
    test("does not classify pseudo-devices as path candidates", async () => {
        const facts = await extractBashFacts(
            "grep -rn needle docs/ 2>/dev/null | head; tee /dev/stderr </dev/stdin >/dev/tty; cat /dev/urandom >/dev/zero",
        );

        expect(facts.pathCandidates).toEqual(["docs/"]);
        expect(facts.redirects).toContainEqual({ operator: ">", target: "/dev/null" });
    });

    test("keeps kernel pseudo-filesystems and block devices as path candidates", async () => {
        const facts = await extractBashFacts("cat /proc/self/environ /sys/class/net /dev/sda");

        expect(facts.pathCandidates).toEqual(["/proc/self/environ", "/sys/class/net", "/dev/sda"]);
    });
});
...
describe("authorization", () => {
    test.each([
        ...
        [
            "git log --all --oneline --decorate --grep='4242\\|unique.*widget\\|widget.*unique' -i -20",
            "allow",
            "allowlist",
        ],
        ["git status; git remote -v", "allow", "allowlist"],
        ["git remote add origin https://example.com/repo.git", "prompt", "unknown"],
        [
            "git status --short && git remote -v && rg -n 'needle' --glob '!graphify-out/**' --glob '!.direnv/**' .",
            "allow",
            "allowlist",
        ],
        ["rg -n 'needle'\n --glob '!graphify-out/**' .", "prompt", "unknown"],
        [
            "git -C /home/example/projects/sample-repo diff --check && git -C /home/example/projects/sample-repo diff -- src/examples/widget.ts",
            "allow",
            "allowlist",
        ],
        ["git -C /tmp/project reset --hard", "prompt", "protected"],
        ["git add src/domain/bash.ts", "allow", "allowlist"],
        ["sd old new src/foo.ts", "allow", "allowlist"],
        ...
        [
            "dotnet tool list --local 2>/dev/null || true; dotnet ef --version 2>&1 | head -5",
            "allow",
            "allowlist",
        ],
