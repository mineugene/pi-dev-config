/**
 * Legacy Bash Gate adapter and compatibility helpers.
 *
 * The active permissions adapter owns authorization. This module retains the
 * former standalone gate API and its narrow subagent approval protocol. Additional
 ...
 * ```
 *
 * When this compatibility adapter is used standalone, Ctrl+Shift+Y and `--yolo`
 * bypass its prompts:
 *
 * ```bash
...
import { subagentBashGatePolicy, subagentMetadata } from "./subagent.ts";

export type { ApprovalRequest } from "./events.ts";
export { subagentBashGatePolicy } from "./subagent.ts";
