 ...
 * tool_call handlers, permissions before rtk so approval sees the command the model
...
import registerPermissions from "./permissions.ts";
...
export const FEATURES: readonly Feature[] = [
    ...
    { name: "prGuard", tier: "core", register: (pi) => registerPrGuard(pi) },
    {
        name: "permissions",
        tier: "core",
        register: (pi, ctx) => registerPermissions(pi, ctx.config),
    },
    ...
    // todo before autoCompact: the compaction policy reads the to-do list to tell
    // a task boundary from a mid-plan stop.
    { name: "todo", tier: "session", register: (pi) => registerTodo(pi) },
    {
        name: "autoCompact",
        tier: "session",
        register: (pi, ctx) => registerAutoCompact(pi, ctx.config),
    },
