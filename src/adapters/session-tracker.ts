...
/**
 * `ExtensionContext.ui` is one shared, session-scoped object: pi core binds it once
 * per session (`AgentSession.bindExtensions` -> `ExtensionRunner.setUIContext`) and
 * hands every extension event handler the same instance. There is no lifecycle event
 * for "a blocking dialog is open", so this wraps the three blocking dialog methods
 * once per session to bracket every call, ours or another extension's, with
 * onStart/onEnd. Wrapping is idempotent per `ui` object in case session_start fires
 * again for the same context.
 */
const instrumentedUis = new WeakSet<ExtensionContext["ui"]>();

function trackBlockingPrompts(
    ui: ExtensionContext["ui"],
    onStart: () => Promise<void>,
    onEnd: () => Promise<void>,
): void {
    if (instrumentedUis.has(ui)) return;
    instrumentedUis.add(ui);

    const originalSelect = ui.select.bind(ui);
    const originalConfirm = ui.confirm.bind(ui);
    const originalInput = ui.input.bind(ui);

    async function bracket<T>(call: () => Promise<T>): Promise<T> {
        await onStart();
        try {
            return await call();
        } finally {
    ...
    ui.select = (...args) => bracket(() => originalSelect(...args));
    ui.confirm = (...args) => bracket(() => originalConfirm(...args));
    ui.input = (...args) => bracket(() => originalInput(...args));
}
...
export default function registerSessionTracker(
    ...
    function derivedState(): AgentPaneState {
        ...
        // A blocking ctx.ui prompt is a certain wait on the human, even mid-turn.
        if (uiPromptWaits > 0) return "needs-input";
    ...
    pi.on("session_start", async (_event, ctx) => {
        ...
        trackBlockingPrompts(
            ...
            async () => {
                uiPromptWaits = Math.max(0, uiPromptWaits - 1);
                await transition();
            },
        );
