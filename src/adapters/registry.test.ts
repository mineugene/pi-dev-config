...
import { EXTENSION_NAMES, FEATURES } from "./registry.ts";
...
describe("feature tiers", () => {
    ...
    it("registers one core permission owner after independent guards", () => {
        const permissions = FEATURES.find((feature) => feature.name === "permissions");
        const names = FEATURES.map((feature) => feature.name);

        expect(permissions?.tier).toBe("core");
        expect(EXTENSION_NAMES).toContain("permissions");
        expect(FEATURES.some((feature) => feature.name === "bashGate")).toBe(false);
        expect(names.indexOf("secretGuard")).toBeLessThan(names.indexOf("permissions"));
        expect(names.indexOf("prGuard")).toBeLessThan(names.indexOf("permissions"));
    });
