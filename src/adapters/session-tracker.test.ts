...
function setup(classifyNeedsInput?: (text: string | undefined) => Promise<boolean>, paneId = "%1") {
    ...
    const ctx = {
        ...
        ui: { notify: vi.fn(), select: vi.fn(), confirm: vi.fn(), input: vi.fn(), setStatus },
    };
    const pi = {
...
describe("Pi session tracker adapter", () => {
    ...
    test("polls tracked-session status at most once every 200 milliseconds", async () => {
        vi.useFakeTimers();
        const harness = setup();
    ...
    test("reports a blocking UI prompt as needing input while the turn runs", async () => {
        const harness = setup();
        // Capture the mock before session_start wraps ctx.ui.select; the wrapper
        // binds to this same underlying mock, so configuring it afterwards still
        // controls what the wrapped call resolves to.
        const selectMock = harness.ctx.ui.select;
        await harness.emit("session_start");
        await harness.emit("agent_start");

        // The implementation only runs after the tracker's onStart hook has
        // reported "needs-input", so asserting inside it proves the wrapper brackets
        ...
        await harness.ctx.ui.select("Pick one", ["a", "b"]);
