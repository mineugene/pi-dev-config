...
export function cleanupWorktree(
    ...
    try {
        ...
        if (status) {
            ...
            // --no-gpg-sign: an automated worktree commit must never block on a
            // signing key (YubiKey touch) with no visible prompt.
            execFileSync("git", ["commit", "--no-verify", "--no-gpg-sign", "-m", commitMsg], {
                cwd: worktree.path,
                stdio: "pipe",
                timeout: 10000,
            });
