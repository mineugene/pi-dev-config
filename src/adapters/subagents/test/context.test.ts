...
describe("buildParentContext", () => {
    ...
    it("uses the latest compaction summary instead of duplicating older messages", () => {
        const out = buildParentContext(
            makeCtx([
                userMsg("orig question"),
                { type: "compaction", summary: "old summary" },
                userMsg("middle question"),
        ...
        expect(out).not.toContain("middle question");
        expect(out.indexOf("[Summary]: latest summary")).toBeLessThan(
            out.indexOf("[Assistant]: follow-up"),
        );
    });

    it("bounds inherited context while keeping the newest content", () => {
        const out = buildParentContext(
            makeCtx([userMsg("old ".repeat(200)), userMsg(`${"x".repeat(200)}-latest`)]),
            300,
        );

        expect(out.length).toBeLessThanOrEqual(300);
        expect(out).toContain("[Earlier parent context truncated.]");
