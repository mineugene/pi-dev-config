...
import type { AgentActivity, Theme } from "../ui/agent-format.ts";
...
function mockSession(messages: unknown[] = []): AgentSession {
    return {
        messages,
...
describe("ConversationViewer", () => {
    describe("render width safety", () => {
        ...
        it("no line exceeds width with running activity indicator", () => {
            const activity: AgentActivity = {
                ...
                session: {
                    getSessionStats: () => ({
                        tokens: { input: 50_000, output: 0, cacheWrite: 0 },
                    }),
                },
                turnCount: 0,
                lifetimeUsage: { input: 50_000, output: 0, cacheWrite: 0 },
            };
            const messages = [
        ...
        it("shows full effective model and thinking level without internal prompt mode", () => {
            ...
            expect(output).toContain("\u{f17a9}  github-copilot/gpt-5.4 · thinking: off");
    ...
    describe("safety net against upstream wrapTextWithAnsi bugs", () => {
        ...
        function callBuildContentLines(
            ...
            return (
                viewer as unknown as { buildContentLines(innerWidth: number): string[] }
    ...
    describe("steer composer", () => {
        ...
        function makeViewer(
            opts: { status?: AgentRecord["status"]; onSteer?: (m: string) => void } = {},
        ) {
