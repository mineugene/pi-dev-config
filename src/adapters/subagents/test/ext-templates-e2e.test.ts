 ...
 * `tools:`/`extensions:` scoping against the REAL pi-mono runtime.
 *
 ...
 *     → real loadCustomAgents()           (frontmatter parsing)
...
describe("tools and extensions — template-driven e2e (real pi-mono, headless)", () => {
    ...
    async function runScenario(agentName: string): Promise<{ active: string[]; prompt: string }> {
        ...
        const modelRegistryMock = {
            find: () => model,
            getAvailable: () => [model],
        } satisfies Pick<ModelRegistry, "find" | "getAvailable">;
        const modelRegistry = modelRegistryMock as unknown as ModelRegistry;
        ...
        const ctxMock = {
            ...
            model,
            modelRegistry,
        } satisfies Pick<ExtensionContext, "cwd" | "getSystemPrompt" | "model" | "modelRegistry">;
        const ctx = ctxMock as unknown as ExtensionContext;
        const piMock = {
            exec: async () => ({ code: 1, stdout: "", stderr: "", killed: false }),
        } satisfies Pick<ExtensionAPI, "exec">;
        const pi = piMock as unknown as ExtensionAPI;
        ...
        // isolated is the one that affects tool gating (forces extensions:false).
    ...
    it("every template on disk is discovered, registered, and self-describing", () => {
        ...
        for (const s of SCENARIOS) {
            ...
            expect(
                s.present.length + s.promptContains.length + s.promptAbsent.length,
            ).toBeGreaterThan(0);
            ...
            expect(
                cfg,
                `template "${s.name}" did not register (parse error or name mismatch?)`,
            ).toBeDefined();
    ...
    it.each(SCENARIOS)(
        ...
        async ({ name, present, absent, promptContains, promptAbsent }) => {
            ...
            for (const tool of present)
                expect(active, `${name}: expected "${tool}" active`).toContain(tool);
            for (const tool of absent)
                expect(active, `${name}: expected "${tool}" NOT active`).not.toContain(tool);
            for (const s of promptContains)
                expect(prompt, `${name}: prompt should contain "${s}"`).toContain(s);
            for (const s of promptAbsent)
                expect(prompt, `${name}: prompt should NOT contain "${s}"`).not.toContain(s);
        },
    );
});
