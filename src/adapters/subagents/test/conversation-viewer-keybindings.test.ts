...
import type { Theme } from "../ui/agent-format.ts";
import { ConversationViewer } from "../ui/conversation-viewer.ts";
import type { ViewerKeybindings } from "../ui/viewer-keys.ts";
import { createViewerKeys } from "../ui/viewer-keys.ts";
...
function scrollOffset(viewer: ConversationViewer): number {
    return (viewer as unknown as { scrollOffset: number }).scrollOffset;
}
