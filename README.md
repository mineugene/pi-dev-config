...
| `routing.presets.<name>.fast` | Optional cheap first response for simple tasks. It is an entry optimisation, not a recovery rung. |
...
| `routing.presets.<name>.cacheTtlMinutes` | Router stickiness for every target in this preset, unless a target sets its own TTL. |
...
| `routing.cacheTtlMinutes` | Router stickiness after a higher route. `0` disables. Target TTL wins over preset TTL, then this value, then the provider-derived default. It does not promise an upstream prompt-cache lifetime. |
...
which is trusted harness configuration. Approving an external file read grants its containing directory for the
...
Bash-specific restrictions above this policy. Project permission settings merge
...
Interactive approvals may be granted once or for the current session; grants are
exact-operation and agent-context scoped, in-memory only, and cleared on session start.
...
| Role | Purpose |
| --- | --- |
| `fast` | Optional cheap first response for a simple task. |
...
model switch. Simple tasks get one fast response. Tool continuations and user follow-ups
move to base. Debugging, multi-step work, reviews, implementation, and refactors start on
base. Fast is only an entry optimisation. Recovery follows `base → escalated → deep`,
...
failure alone. A matching compiler, test, or runtime signature that persists after an
edit becomes stagnation and can advance the recovery ladder. Footer routing status
uses explicit `fast`, `base`, `escalated`, `deep`, or `manual override` labels with
concise reasons such as `multi-step task`, `assistant failure threshold reached`, or
`stagnation threshold reached`.

The router never de-escalates within a subtask. Across task boundaries it keeps a
higher route while its configured router-stickiness TTL is active, then permits a
downshift. `/routing-role fast|base|escalated|deep` forces a semantic role for the current
subtask; `/routing-role auto` and `/routing-auto` clear role and exact-model overrides.
A manual exact-model change remains available and pauses automatic routing for that
subtask. Completed `todo` plans provide an exact boundary, with explicit completion or
a non-continuation user instruction as the fallback. Finished subtasks persist only
redacted routing metrics, and `/usage` shows aggregate routing completion, recovery,
token, and cost data.
...
- **Proactive compaction** (`autoCompact`): compact when the agent settles rather than
  when the context overflows. The threshold decides whether the context is large enough;
  the `todo` list decides when it is safe. In the soft band an unfinished plan defers
...
- **Model routing** (`routing`): route simple work to fast and complex or continuing work
  to base, then recover through optional escalated and deep targets. Repeated remediated
  development failures become stagnation; ordinary tool and development failures do not
  escalate alone.
  Target-level warm-route TTLs prevent needless churn. `/routing-preset` swaps named
  model sets, `/routing-role` forces semantic difficulty, and `/routing-auto` clears
  role and exact-model overrides; the footer shows the preset, role, and decision reason.
...
- **Permissions** (`permissions`): evaluate normalized tool effects under `ask`,
  `auto`, or `bypass`; prompt interactively and fail closed in headless runs.
  Bash-specific `bashGate.rules` remain restrictive overlays.
...
- `/routing-role <fast|base|escalated|deep|auto>` or `/route ...`: force a semantic routing role for the current subtask, or resume automatic routing.
